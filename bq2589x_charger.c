...
static int bq2589x_init_device(struct bq2589x *bq)
{
	...
	if(ret < 0){
		dev_err(bq->dev,"%s:Failed to set charger charge current:%d\n",__func__,ret);
		return ret;
	}
...
static void bq2589x_check_if_tuneup_workfunc(struct work_struct* work)
{
	...
	if(bq->vbat_volt > bq->vbat_min_volt_to_tuneup && bq->vbat_volt < 4100 /*bq->rsoc < 95*/){    
        dev_info(bq->dev,"%s:trying to tune up vbus voltage\n",__func__);
		voltcontrol.TuneTargetVolt = bq->vbus_volt_high_level;
    ...
    else if(bq->vbat_volt > 4100/*bq->rsoc >95*/)
        schedule_delayed_work(&bq->ico_work,0);
	else
...
static void bq2589x_tune_volt_workfunc(struct work_struct * work)
{
    ...
    dev_info(bq->dev,"%s:vbus voltage:%d, Tune Target Volt:%d\n",__func__,bq->vbus_volt,voltcontrol.TuneTargetVolt);

	if( (voltcontrol.toTuneUpVolt && bq->vbus_volt > voltcontrol.TuneTargetVolt)|| 
		(voltcontrol.toTuneDownVolt && bq->vbus_volt < voltcontrol.TuneTargetVolt)){
...
static void bq2589x_monitor_workfunc(struct work_struct *work)
{
	...
	if(bq->vbus_type == BQ2589X_VBUS_USB_DCP && bq->vbus_volt > bq->vbus_volt_high_level
		&& bq->rsoc > 95 /*bq->vbat_volt > 4180*/ && !voltcontrol.toTuneDownVolt){
		voltcontrol.toTuneDownVolt = true;
		voltcontrol.toTuneUpVolt = false;
		voltcontrol.TuneTargetVolt = bq->vbus_volt_low_level;
		voltcontrol.TuneVoltDone = false;
		voltcontrol.TuneCounter = 0;
		voltcontrol.TuneFail = false;
		schedule_delayed_work(&bq->volt_tune_work,0);
			   ...
			   const struct i2c_device_id *id)
{
    ...
    g_bq->vbus_volt_high_level = 4400; //by default adapter output 5v, if >4.4v,it is ok after tune up
    g_bq->vbus_volt_low_level = 5500; //by default adapter output 5v, if <5.5v,it is ok after tune down
    g_bq->vbat_min_volt_to_tuneup = 3000; // by default, tune up adapter output only when bat is >3000

//    g_bq1->charge_voltage = 4208;
//    g_bq1->charge_current = 2048;

	if(client->dev.of_node)
		 bq2589x_parse_dt(&client->dev, g_bq);

	ret = bq2589x_init_device(g_bq);
	if (ret) {
...
static struct i2c_board_info __initdata i2c_bq2589x_charger[] =
{
    { 
        I2C_BOARD_INFO("bq2589x",0x6A),
    },
};
...
static int __init bq2589x_charger_init(void)
{
    ...
    i2c_register_board_info(0,i2c_bq2589x_charger,ARRAY_SIZE(i2c_bq2589x_charger));

    if(i2c_add_driver(&bq2589x_charger_driver)){
        printk("%s, failed to register bq2589x_charger_driver.\n",__func__);
    }
    else{
        printk("%s, bq2589x_charger_driver register successfully!\n",__func__);
    }
